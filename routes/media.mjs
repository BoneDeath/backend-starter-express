...
router.get("/", async (req, res) => {
  try {
    let results = await Media.find({}).limit(10).exec();
...
router.post("/", async (req, res) => {
  try {
    ...
    const savedData = await new Media(bodyData).save();
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const updatedData = await Media.findByIdAndUpdate(dataId, updateData, {
      new: true,
    });
    if (!updatedData) {
...
router.delete("/:id", async (req, res) => {
  try {
    const dataId = req.params.id;
    const deletedData = await Media.findByIdAndDelete(dataId);
    if (!deletedData) {
      return res.status(404).json({ message: "Data tidak ditemukan" });
    }
